...
static void receivePrint(int sfd)
{
    ...
    if (sockDataSize < 0)
    {
        cli::printErrno("recvfrom() failed", errno);
...
// looks like this is always the AF used when creating the socket
        ...
        if (ethProtocol == ETH_P_IP)
        {
            ...
            // filter
            // if (ipProtocol == IPPROTO_TCP) { return; }
        ...
        else if (ethProtocol == ETH_P_8021Q)
        {
            ...
            if (ethProtocol != ethProtocolEff)
            {
                printf("  effective proto \033[31m0x%04x %s\033[39m\n", (int)ethProtocolEff, ethptos(ethProtocolEff).c_str());
            }
...
void receivePrintL2()
{
    std::lock_guard<std::mutex> lg(___mtx);
